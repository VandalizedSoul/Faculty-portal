...
def updateImage(request):
  faculty_id = request.POST.get('id', '')
  image = request.FILES.get('image')
  faculty = Faculty.objects.filter(faculty_name=faculty_id).update(image=image)
...
def updateEmail(request):
  ...
  faculty = Faculty.objects.filter(faculty_name=faculty_id).update(email=email)
...
def updateAwards(request):
  ...
  for award in awards:
    ...
    a.award_url = award[4]
    arr.append(a)
  faculty = Faculty.objects.filter(faculty_name=faculty_id).update(awards=arr)
...
def updateOrganizations(request) :
  ...
  for organization in organizations : 
    ...
    arr.append(o)
  faculty = Faculty.objects.filter(faculty_name=faculty_id).update(organizations=arr)
...
def updateSubjects(request):
  ...
  for subject in subjects : 
    ...
    arr.append(t)
  faculty = Faculty.objects.filter(faculty_name=faculty_id).update(subjects=arr)
